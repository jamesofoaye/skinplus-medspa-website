...
exports.send_SMS_At_Eight_Am = functions.pubsub
...
exports.send_SMS_At_One_Pm = functions.pubsub
...
const getAppointments = async () => {
  ...
  snapshot.forEach((doc) => {
    ...
    const message = `Hello ${
      doc.data().name
    }, this is a reminder that you have ${doc
      .data()
      .services.map(
        (service) => service
      )} appointment at SkinPlus Medspa Today at
      ${moment(doc.data().nextAppointmentDate).format(
        'LT'
      )}. Visit ${`https://skinplusofficial.com/appointment/${
      doc.data().userId
    }`} to Confirm, Cancel or Reschedule your appointment. See you soon!`;

    const cancelledAndRescheduled = doc.data().status;

    const conditionsToSend = async () => {
      if (
        cancelledAndRescheduled === 'cancelled' ||
        cancelledAndRescheduled === 'rescheduled'
      ) {
        return null;
      } else if (
        moment(doc.data().nextAppointmentDate).format('L') ===
        moment().format('L')
      ) {
        ...
        functions.logger.info(
          `Phone number: ${doc.data().phone} and message: ${message}`,
          { structuredData: true }
        );
      }
    };
...
const getAppointmentsForOnePmSMS = async () => {
  ...
  snapshot.forEach((doc) => {
    ...
    const formattedNextAppointmentDate = moment(
      doc.data().nextAppointmentDate
    ).format('L');

    const formattedToday = moment(today, 'MM-DD-YYYY')
      .add(2, 'days')
      .format('L');
    ...
    const twoDaysBeforeReminder = `Hello ${
      doc.data().name
    }, this is a reminder that you have ${doc
      .data()
      .services.map((service) => service)} appointment at SkinPlus Medspa on 
      ${moment(
        doc.data().nextAppointmentDate
      ).calendar()}. Visit ${`https://skinplusofficial.com/appointment/${
      doc.data().userId
    }`} to Confirm, Cancel or Reschedule your 
    ...
    const conditionsToSendOnePmSMS = async () => {
      if (
        ...
        functions.logger.info(
          `Phone number: ${
            doc.data().phone
          } and message: ${twoDaysBeforeReminder}`,
          { structuredData: true }
        );
      }
    };
